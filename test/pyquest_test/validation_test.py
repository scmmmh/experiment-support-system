...
def page_schema_test():
    ...
    eq_({'items': {'1': {'name': 'Mark', 'email': 'somebody@somewhere.com', 'homepage': 'http://www.example.com'}}},
        PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(submission))
    submission = MultiDict([('items.1.name', ''),
                            ('items.1.email', 'somebody@somewhere.com'),
                            ('items.1.homepage', 'http://www.example.com')])
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, submission)
    submission = MultiDict([('items.1.name', 'Mark'),
                            ('items.1.email', ''),
                            ('items.1.homepage', 'http://www.example.com')])
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, submission)
...
def number_input_test():
    ...
    eq_({'number': {'type': 'number', 'required': True}},
        schema)
    PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.number', '4')]))
    ...
    eq_({'number': {'type': 'number', 'min_value': 2, 'max_value': 6, 'step': 2}},
        schema)
    PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.number', '4')]))
    PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.number', '2')]))
    PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.number', '6')]))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.number', '1')]))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.number', '9')]))
...
def email_input_test():
    ...
    eq_({'email': {'type': 'email', 'required': True}},
        schema)
    PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.email', 'somebody@example.com')]))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.email', 'somebody.somewhere')]))
...
def url_input_test():
    ...
    eq_({'url': {'type': 'url', 'required': True}},
        schema)
    PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.url', 'http://example.com/home/~username?yes=q')]))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.url', 'Nothing')]))
...
def date_input_test():
    ...
    eq_({'items': {'1': {'birthday': datetime.date(1964, 2, 29)}}},
         PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.birthday', '29/02/1964')])))
    eq_({'items': {'1': {'birthday': datetime.date(1964, 2, 29)}}},
         PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.birthday', '29/2/1964')])))
    eq_({'items': {'1': {'birthday': datetime.date(1964, 2, 2)}}},
         PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.birthday', '2/2/1964')])))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.birthday', '29/13/1967')]))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.birthday', '29/02/1967')]))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.birthday', 'Something')]))
...
def time_input_test():
    ...
    eq_({'items': {'1': {'time': datetime.time(14, 32)}}},
         PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.time', '14:32')])))
    eq_({'items': {'1': {'time': datetime.time(4, 32)}}},
         PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.time', '4:32')])))
    eq_({'items': {'1': {'time': datetime.time(12, 3)}}},
         PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.time', '12:3')])))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.time', '26:12')]))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.time', '12:65')]))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.time', 'Something')]))
...
def datetime_input_test():
    ...
    eq_({'items': {'1': {'now': datetime.datetime(1964, 2, 29, 14, 32)}}},
         PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.now', '29/02/1964 14:32')])))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.now', '29/02/1967 13:12')]))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.now', '29/02/1968 27:12')]))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.now', 'Something')]))
...
def month_input_test():
    ...
    eq_({'month': {'type': 'month', 'required': True}},
        schema)
    eq_({'items': {'1': {'month': 4}}},
         PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.month', '4')])))
    eq_({'items': {'1': {'month': 4}}},
         PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.month', 'apr')])))
    eq_({'items': {'1': {'month': 4}}},
         PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.month', 'april')])))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.month', '13')]))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.month', 'frank')]))
...
def short_text_input_test():
    ...
    eq_({'name': {'type': 'unicode', 'required': True}},
        schema)
    PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.name', 'Sir Lancelot')]))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.name', '')]))
...
def long_text_input_test():
    ...
    eq_({'description': {'type': 'unicode', 'required': True}},
        schema)
    PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.description', 'I am a knight of the round table')]))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.name', '')]))
...
def rating_test():
    ...
    eq_({'rating': {'type': 'single_in_list', 'values': ['1', '2', '3']}},
        schema)
    PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.rating', '2')]))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.rating', '2'), ('items.1.rating', '3')]))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.rating', '0')]))
...
def rating_group_test():
    ...
    eq_({'rating': {'type': 'compound',
        ...
        schema)
    PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.rating.q1', '2')]))
    PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.rating.q1', '2'), ('items.1.rating.q2', '1')]))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.rating', '2'), ('items.1.rating', '3')]))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.rating', '0')]))
    ...
    eq_({'rating': {'type': 'compound',
                    ...
                    'required': True}},
        schema)
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.rating.q1', '2')]))
...
def listchoice_test():
    ...
    eq_({'choose': {'type': 'single_in_list', 'values': ['1', '2', '3']}},
        schema)
    PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.choose', '2')]))
    PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.choose', '')]))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.choose', '2'), ('items.1.choose', '3')]))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.choose', '0')]))
...
def multichoice_test():
    ...
    eq_({'choose': {'type': 'multi_in_list', 'values': ['1', '2', '3']}},
        schema)
    PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.choose', '2')]))
    PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.choose', '2'), ('items.1.choose', '3')]))
    PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.choose', '')]))
    PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1', '')]))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.choose', '2'), ('items.1.choose', '4')]))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.choose', '0')]))
...
def multichoice_group_test():
    ...
    eq_({'choose': {'type': 'compound',
        ...
        schema)
    PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.choose.q1', '2')]))
    PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.choose.q1', '2'), ('items.1.choose.q1', '3'), ('items.1.choose.q2', '3'), ('items.1.choose.q2', '1')]))
    PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.choose.q1', '')]))
    PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1', '')]))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.choose.q1', '2'), ('items.1.choose.q1', '4')]))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.choose.q1', '1'), ('items.1.choose.q2', '0')]))
    ...
    eq_({'choose': {'type': 'compound',
        ...
        schema)
    PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.choose.q1', '2'), ('items.1.choose.q1', '3'), ('items.1.choose.q2', '3'), ('items.1.choose.q2', '1')]))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.choose.q1', '2')]))
...
def confirm_test():
    ...
    eq_({'items': {'1': {'confirmation': True}}},
        PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.confirmation', 'true')])))
    eq_({'items': {'1': {'confirmation': False}}},
        PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1', '')])))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.confirmation', 'nonsense')]))
    ...
    eq_({'items': {'1': {'confirmation': True}}},
        PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.confirmation', 'true')])))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.confirmation', '')]))
...
def ranking_test():
    ...
    eq_({'items': {'1': {'importance': {'dog': 1, 'cat': 0, 'mouse': 2}}}},
        PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.importance.dog', '1'), ('items.1.importance.cat', '0'), ('items.1.importance.mouse', '2')])))
    PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.importance', '')]))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.importance.dog', '0'), ('items.1.importance.cat', '1')]))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.importance.dog', '0'), ('items.1.importance.cat', '1'), ('items.1.importance.cat', '3')]))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.importance.dog', '0'), ('items.1.importance.dog', '1'), ('items.1.importance.cat', '1'), ('items.1.importance.cat', '2')]))
    ...
    eq_({'items': {'1': {'importance': {'cat': 0}}}},
        PageSchema(schema, [{'did': 1}], csrf_test=False).to_python(MultiDict([('items.1.importance.cat', '0')])))
    assert_raises(Invalid, PageSchema(schema, [{'did': 1}], csrf_test=False).to_python, MultiDict([('items.1.importance.dog', '0')]))
