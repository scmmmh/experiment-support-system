...
def upgrade():
    ...
    for qsheet in op.get_bind().execute(qs.select()):
        ...
        if data_items.rowcount > 0:
            ...
            ds_pk = op.get_bind().execute(ds.insert().values(name='created by migrations for qsheet %d' % qsheet.id,
                                                             owned_by=survey.owned_by,
                                                             survey_id=qsheet.survey_id)).inserted_primary_key[0]
            for data_item in data_items:
                op.get_bind().execute(di.update().where(di.c.id == data_item.id).values(dataset_id=ds_pk,
                                                                                        qsheet_id=None))
...
def downgrade():
    ...
    for qsheet in op.get_bind().execute(qs.select()):
        if qsheet.dataset_id:
            ...
            for data_item in data_items:
                op.get_bind().execute(di.update().where(di.c.id == data_item.id).values(dataset_id=None,
                                                                                        qsheet_id=qsheet.id))
