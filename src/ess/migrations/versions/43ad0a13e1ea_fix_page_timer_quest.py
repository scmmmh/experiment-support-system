...
def upgrade():
    ...
    op.get_bind().execute(qt.update(sa.and_(qt.c.group_id==core_qtg[0], qt.c.name==u'page_timer'),
                                    {'frontend': """${f.hidden_field(name, '0', class_='role-timer')}"""}))
    op.get_bind().execute(qt.update(sa.and_(qt.c.group_id==core_qtg[0], qt.c.name==u'auto_commit'),
