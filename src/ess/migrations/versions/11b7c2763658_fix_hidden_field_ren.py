...
def upgrade():
    ...
    qtg_id = op.get_bind().execute(qtg.select().where(sa.and_(qtg.c.parent_id == qtg_id,
                                                              qtg.c.name == 'hidden'))).first()[0]
    op.get_bind().execute(qt.update().
                          where(sa.and_(qt.c.group_id == qtg_id, qt.c.name == 'hidden_value')).
                          values({'frontend': "${f.hidden_field(name, sub(q.attr_value('further.value'), i, p))}"}))
...
def downgrade():
    ...
    op.get_bind().execute(qt.update().
                          where(sa.and_(qt.c.group_id == qtg_id, qt.c.name == 'hidden_value')).
                          values({'frontend':
                                  "${f.hidden_field(name, sub(q.attr_value('further.value'), i, p), None)}"}))
