...
def upgrade():
    ...
    for participant in op.get_bind().execute(participants.select()):
        ...
        for question in op.get_bind().execute(questions.select().
                                              select_from(questions.join(qsheets,
                                                                         questions.c.qsheet_id == qsheets.c.id)).
                                              where(qsheets.c.survey_id == participant.survey_id)):
            if question.required:
                if not op.get_bind().execute(answers.select().
                                             where(sa.and_(answers.c.participant_id == participant.id,
                                                           answers.c.question_id == question.id))).first():
                    completed = False
        op.get_bind().execute(participants.update().values(completed=completed).
