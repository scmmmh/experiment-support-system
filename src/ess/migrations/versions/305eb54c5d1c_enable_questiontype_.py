...
def upgrade():
    ...
    op.get_bind().execute(perm.insert(values={'name': 'admin.question_types',
                                              'title': 'Administer the question types'}))
    op.add_column('question_types', sa.Column('enabled', sa.Boolean))
    op.add_column('question_types', sa.Column('order', sa.Integer))
    op.get_bind().execute(qt.update(values={'enabled': True}))
    op.add_column('question_type_groups', sa.Column('enabled', sa.Boolean))
    op.get_bind().execute(qtg.update(values={'enabled': True}))


def downgrade():
    op.get_bind().execute(perm.delete(perm.c.name == 'admin.question_types'))
