...
def upgrade():
    ...
    op.create_table('question_type_groups',
                    ...
                    sa.Column('title', sa.Unicode(255)),
                    sa.Column('order', sa.Integer))
    op.add_column('question_types', sa.Column('group_id',
                                              sa.Integer,
                                              sa.ForeignKey('question_type_groups.id',
                                                            name='question_type_groups_fk')))
    qtg_pk = op.get_bind().execute(qtg.insert().values(name=u'core', title=u'Core Questions', order=0)).\
