...
qt = sa.Table('qsheet_transitions', metadata,
              sa.Column('id', sa.Integer, primary_key=True),
              sa.Column('source_id', sa.Integer, sa.ForeignKey('qsheets.id',
                                                               name='qsheet_transitions_qsheets_source_fk')),
              sa.Column('target_id', sa.Integer, sa.ForeignKey('qsheets.id',
                                                               name='qsheet_transitions_qsheets_target_fk')),
...
def upgrade():
    ...
    for row in op.get_bind().execute(sa.select([tc.c.expected_answer, qt.c.id, q.c.name, qs.c.name],
                                               sa.and_(qt.c.id == tc.c.transition_id,
