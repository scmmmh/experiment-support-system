...
def upgrade():
    op.create_table('data_set_attribute_keys',
                    ...
                    Column('order', Integer),
                    Column('dataset_id', Integer, ForeignKey('data_sets.id',
                                                             name='data_set_attribute_keys_dataset_id_fk')))

    op.add_column('data_item_attributes',
                  Column('key_id', Integer, ForeignKey('data_set_attribute_keys.id',
                                                       name='data_item_attributes_data_set_attribute_key_fk')))
    ...
    for dataset in op.get_bind().execute(ds.select()):
        ...
        if len(data_items) > 0:
            ...
            for data_item in data_items:
                ...
                for dia in dias:
                    dsak = op.get_bind().execute(dk.select().where(and_(dk.c.dataset_id == data_item.dataset_id,
                                                                        dk.c.key == dia.key))).first()
