...
class ParticipantManager(object):
    ...
    def state(self):
        ...
        if not state:
            state = {}
        if 'current_qsheet' not in state:
            state['current_qsheet'] = self._survey.start.id
        self._participant.set_state(state)
        return state
    
    def current_qsheet(self):
        state = self.state()
        return self._dbsession.query(QSheet).filter(QSheet.id==state['current_qsheet']).first()
    ...
    def next_qsheet(self):
        for transition in sorted(self.current_qsheet().next, key=transition_sorter, reverse=True):
            ...
            if (transition.target and (condition == None or condition.evaluate(self._dbsession, self.participant()) == True)):
                state = self.state()
                state['current_qsheet'] = transition.target.id
                self._participant.set_state(state)
                return transition.target
        state = self.state()
        state['current_qsheet'] = '_finished'
        self._participant.set_state(state)
        return None
...
def run_survey(request):
    ...
    if survey:
        ...
        if part_manager.state()['current_qsheet'] == '_finished':
        ...
        if request.method == 'GET':
            ...
            try:
                ...
                with transaction.manager:
                    ...
                    part_manager.next_qsheet() # TODO: Fix Single / Repeat pages
...
def finished_survey(request):
    ...
    if survey:
        ...
        return {'survey': survey,
                'control': 0}#calculate_control_items(None, get_participant(dbsession, survey, init_state(request, dbsession, survey)))} TODO: Fix
    else:
        raise HTTPNotFound()
